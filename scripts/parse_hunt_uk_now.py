#!/usr/bin/env python3
"""Quick Hunt UK parser from web_extract markdown into docs/data/hunt_uk_jobs.json"""
import json, os, re
...
- **Employer**: Gaubert's Brothers Limited (posted on behalf of Capgemini, an inclusive myGwork member and LGBTQ+ business community platform partner)
...
- **Employer**: Gaubert's Brothers Limited (posted on behalf of Capgemini/myGwork)
...
- **Employer**: DB Group Services (UK) Limited (Deutsche Bank)
- **Location**: London, England, United Kingdom
- **Posted**: 6/1/2026
...
for block in blocks:
    ...
    sn_role = 'servicenow' in title.lower()
    role_type = 'unknown'
    t = title.lower()
    if 'architect' in t:
        role_type = 'architect'

#!/usr/bin/env python3
"""Gradcracker Scraper using Playwright"""
...
class GradcrackerScraper:
    ...
    async def scrape_jobs(self):
        try:
            ...
            search_url = "https://www.gradcracker.com/search/jobs?q=ServiceNow"
            print(f"[Gradcracker] Navigating to: {search_url}")
            ...
            print(f"[Gradcracker] Page length: {len(content)} characters")
            
            with open('/tmp/gradcracker_page.html', 'w') as f:
                f.write(content)
            print("[Gradcracker] Page saved to /tmp/gradcracker_page.html")
            ...
            print(f"[Gradcracker] Found {len(job_cards)} job cards")
            ...
            for card in job_cards[:50]:
                try:
                    ...
                    print(f"Error parsing Gradcracker card: {e}")
                    continue
            
            await self.close()
            
        except Exception as e:
            print(f"Error scraping Gradcracker: {e}")
            import traceback
            traceback.print_exc()
    ...
    async def run(self):
        try:
            ...
            print(f"Error in GradcrackerScraper.run: {e}")
            import traceback
            traceback.print_exc()
            return []

#!/usr/bin/env python3
"""Milkround Scraper using Playwright"""
...
class MilkroundScraper:
    ...
    async def scrape_jobs(self):
        try:
            ...
            search_url = "https://www.milkround.com/jobs/search?q=ServiceNow"
            print(f"[Milkround] Navigating to: {search_url}")
            ...
            print(f"[Milkround] Page length: {len(content)} characters")
            
            with open('/tmp/milkround_page.html', 'w') as f:
                f.write(content)
            print("[Milkround] Page saved to /tmp/milkround_page.html")
            ...
            print(f"[Milkround] Found {len(job_cards)} job cards")
            ...
            for card in job_cards[:50]:
                try:
                    ...
                    print(f"Error parsing Milkround card: {e}")
                    continue
            
            await self.close()
            
        except Exception as e:
            print(f"Error scraping Milkround: {e}")
            import traceback
            traceback.print_exc()
    ...
    async def run(self):
        try:
            ...
            print(f"Error in MilkroundScraper.run: {e}")
            import traceback
            traceback.print_exc()
            return []
if __name__ == "__main__":
    import asyncio
    scraper = MilkroundScraper()
    result = scraper.run()
